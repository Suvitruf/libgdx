...
public class Oneal extends NpcBase {
	
	public static String Name =  "Oneal";
	...
	public void changeDirection(float delta) {
		...
		if(seeBomberman()){ 
			...
			if(bX == X){
				...
				if(getPosition().x - X>0 && getPosition().y - (int)getPosition().y>offset){
				...
				//if(bY>Y) {
				if(bY>Y && getPosition().x - (int)getPosition().x>offset) {
					direction = Direction.UP;
					getVelocity().y = SPEED;//checkColision();	return;
				} 
				//else{
				else if (getPosition().x - (int)getPosition().x>offset){
	...
	public Oneal(World world, Vector2 pos){ //constructor takes parameter vector2 which is a position (x,y)
		super(pos);
		this.world = world;
		//getVelocity().y =SPEED;	
		//state = State.NONE;
